...
int main() {
    ...
    std::array<std::vector<double>,2> du = {
        std::vector<double>(n_fault_dofs, slip),   
        std::vector<double>(n_fault_dofs, slip)
    };

    std::array<std::vector<double>,2> all_dofs_rhs = {
    ...
    for (int k = 0; k < 2; k++) {
        for (int j = 0; j < 2; j++) {
            Problem<2> p = {fault, surface, ek.hypersingular_mat[k][j], du[j]};
    ...
    hdf_out("2dthrust0.hdf5", surface, soln[0]); 
    hdf_out("2dthrust1.hdf5", surface, soln[1]); 
}
