...
def test_planestrain():
    ...
    surface = line_mesh([-100, 0], [100, 0]).refine_repeatedly(10)

    hyp = ElasticHypersingular(30e9, 0.25)
    soln = solve(2, surface, fault, hyp)

    x = get_vertices(2, surface, 0)
    indices = [i for i in range(x.shape[0])
               if 0 < np.abs(x[i]) < 10]
    exact_ux, exact_uy = exact_displacements(x[indices])
    ux = soln[0][indices]
    uy = soln[1][indices]
    ux_error = np.sqrt(np.mean((ux - exact_ux) ** 2))
    uy_error = np.sqrt(np.mean((ux - exact_ux) ** 2))
    assert(ux_error < 6e-4)
    assert(uy_error < 6e-4)
